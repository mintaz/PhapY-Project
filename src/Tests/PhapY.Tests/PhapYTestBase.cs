...
using PhapY.Migrations.SeedData;
...
namespace PhapY.Tests
{
    public abstract class PhapYTestBase : AbpIntegratedTestBase<PhapYTestModule>
    {
        ...
        protected void UsingDbContext(Action<PhapYDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PhapYDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PhapYDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PhapYDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PhapYDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhapYDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PhapYDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhapYDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PhapYDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PhapYDbContext, Task<T>> func)
        {
            T result;
